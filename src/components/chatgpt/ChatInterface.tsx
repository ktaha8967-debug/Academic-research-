...
export function ChatInterface({
  ...
  return (
    <div className="relative flex flex-col flex-1 h-full overflow-hidden bg-background/50 backdrop-blur-xs z-10">
      {/* 1. RESEARCH PROGRESSION BAR (Subtle solid-color workflow tracker) */}
      <div className="relative z-10 border-b border-border/80 bg-card/75 backdrop-blur-md px-4 py-2 shrink-0 overflow-x-auto [scrollbar-width:none]">
      ...
      <div className="relative z-10 flex-1 overflow-y-auto px-4 py-6 md:px-8 space-y-6 pb-44">
        {messages.length === 0 ? (
          <div className="max-w-3xl mx-auto space-y-8 pt-4">
            {/* Academic Workspace Welcome Header */}
            <div className="text-center space-y-2.5 animate-in fade-in zoom-in-95 duration-500">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-border bg-card/90 backdrop-blur-xs text-xs font-semibold text-muted-foreground mb-1 shadow-xs hover:border-primary/40 transition-colors">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-primary opacity-75" />
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-primary" />
                </span>
                <span className="text-foreground font-medium">Citation-Grounded Research Engine &bull; Live Federated Mesh</span>
              </div>
            ...
            {/* Quick-Start Workflow Tiles (With interactive hover elevation) */}
              ...
              {STARTER_PROMPTS.map((prompt, idx) => {
                ...
                return (
                  <button
                    ...
                    className="card-academic-interactive p-4 rounded-xl border border-border bg-card/90 backdrop-blur-xs text-left group flex flex-col justify-between"
                  >
                    <div className="space-y-1.5">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="p-1 rounded-md bg-muted/60 text-muted-foreground group-hover:text-primary group-hover:bg-primary/10 transition-colors">
                    ...
                    <div className="mt-3 flex items-center gap-1 text-[11px] font-bold text-primary opacity-75 group-hover:opacity-100 group-hover:translate-x-0.5 transition-all">
                    ...
                    <div className="mt-4 pt-3 border-t border-border/80 space-y-3">
                      ...
                      <div className="space-y-2">
                        {msg.structuredData.researchGaps.map((gap: StructuredResearchGap) => (
                          <div
                            key={gap.id}
                            className="card-academic-interactive rounded-xl border border-border bg-card p-3.5 space-y-2"
                          >
                            ...
                            <div className="pt-2 border-t border-border/40 flex items-center justify-between">
                              ...
                              <button
                                ...
                                className="button-interactive inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-primary/10 hover:bg-primary/20 text-primary text-xs font-bold transition-colors"
                              >
                    ...
                    <div className="mt-4 pt-3 border-t border-border/80 space-y-3">
                      ...
                      <div className="space-y-2">
                        {msg.structuredData.researchQuestions.map((q: StructuredResearchQuestion) => (
                          <div
                            key={q.id}
                            className="card-academic-interactive rounded-xl border border-border bg-card p-3.5 space-y-2.5"
                          >
                            <div className="flex items-start justify-between gap-2">
                            ...
                            <div className="pt-2 border-t border-border/40 flex items-center justify-between">
                              ...
                              <button
                                ...
                                className="button-interactive inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 text-xs font-bold transition-colors"
                              >
                      ...
                      <div className="grid grid-cols-1 gap-2.5">
                        {msg.sources.map((src) => {
                          ...
                          return (
                            <div
                              key={src.id}
                              className="card-academic-interactive rounded-xl border border-border bg-card p-3 space-y-2"
                            >
                              <div className="flex items-start justify-between gap-2">
                                <a
      ...
      <div className="absolute inset-x-0 bottom-0 z-30 p-3 sm:p-5 bg-background/90 backdrop-blur-md border-t border-border/80">
        <div className="max-w-3xl mx-auto w-full space-y-2">
          {/* Active Project & Attached Papers Pills */}
          <div className="flex items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
            {activeProject && (
              <span className="inline-flex items-center gap-1.5 rounded-full bg-card px-2.5 py-0.5 text-[11px] font-bold text-foreground shrink-0 border border-border shadow-xs">
            ...
            <button
              ...
              className="button-interactive inline-flex items-center gap-1 rounded-full border border-border hover:border-primary px-2.5 py-0.5 text-[11px] font-medium text-muted-foreground hover:text-foreground bg-card transition-colors shrink-0 shadow-xs"
            >
          ...
          {/* Main Input Box */}
          <div className="relative flex flex-col rounded-2xl border border-input bg-card/95 backdrop-blur-xs focus-within:border-primary focus-within:shadow-md transition-all">
            <textarea
              ref={textareaRef}
              value={inputText}
